...
```bash
get-sesami$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
get-sesami$ sam local start-api
get-sesami$ curl http://localhost:3000/
```
...
```bash
get-sesami$ sam logs -n HelloWorldFunction --stack-name get-sesami --tail
```
